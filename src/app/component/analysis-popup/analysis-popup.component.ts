...
//         break;
  ...
  changePlotAnalysis() {
    switch (AnalysisPopupComponent.selectedAnalysis) {
          ...
          case 'Newton\'s II Law':
            ...
            switch (this.forceAnalysis.selectedAnalysis) {
              case 'none':
                ...
                break;
              case 'Joint Forces':
                if (this.forceMagPlot.force === 'none') {
                  this.showChart = false;
                  return;
                }
                this.showChart = true;
                this.determineChart(AnalysisPopupComponent.selectedAnalysis, analysisType, this.forceAnalysis.selectedAnalysis, this.forceMagPlot.force);
                break;
              case 'Input Torque':
                this.showChart = true;
                this.determineChart(AnalysisPopupComponent.selectedAnalysis, analysisType, this.forceAnalysis.selectedAnalysis, '');
                break;
              default:
                this.showChart = false;
      ...
      case 'stress':
        break;
      case 'kinematic':
        switch (this.kinematicAnalysis.selectedMechanismState) {
          case 'loop':
          case 'ic':
            switch (this.kinematicAnalysis.selectedMechanismState) {
              case 'loop':
              case 'ic':
                switch (this.kinematicPropertyAnalysis.selectedAnalysis.substring(0, 12)) {
                  case 'Linear Joint':
  ...
  //   // TODO: Determine different logic where this.showCharts is not always set up as false. Probably just put this within initializer
  //   // TODO: Change the logic a bit to be easier to follow
  //   this.showChart = false;
  //   let analysisString = '';
  //   switch (more_type) {
  //     case 'Static Equilibrium':
  ...
  // analysis = Input Torque, Joint Forces, Linear Joint Pos ...
  // type_of = joint/link/force id
  // more_type = statics, dynamics, loop, ic
  // determineAnalysis(analysis: string, type_of: string, more_type: string): [[number[], number[], number[]], string[]] {
  determineAnalysis(analysis: string, analysisType: string, mechProp: string, mechPart: string): [[number[], number[], number[]], string[]] {
